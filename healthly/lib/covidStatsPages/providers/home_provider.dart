...
class HomeProvider extends ChangeNotifier {
  ...
  double getDialValue(int dialIndex) {
    if (dialIndex == 0) {
      return double.parse(
          (_totalActive / _totalConfirmed).toStringAsPrecision(3));
    } else if (dialIndex == 1) {
      return double.parse(
          (_totalDead / _totalConfirmed).toStringAsPrecision(3));
    } else {
      return double.parse(
          (_totalRecovered / _totalConfirmed).toStringAsPrecision(3));
    }
  }
      ...
      data.forEach((countrySummary) {
        if (countrySummary['Country'] != '') {
          ...
          if (tempTopSix.isEmpty) {
            tempTopSix.add(temp);
          } else if (tempTopSix[tempTopSix.length - 1].totalConfirmed <
                  temp.totalConfirmed &&
              tempTopSix.length < 6) {
            tempTopSix.add(temp);
          } else if (tempTopSix[tempTopSix.length - 1].totalConfirmed <
