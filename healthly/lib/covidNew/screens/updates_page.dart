...
class UpdatesScreen extends StatefulWidget {
  ...
  UpdatesScreen({Key key, this.imgPath, this.color, this.isHealth = false})
      : super(key: key);
...
class _UpdatesScreenState extends State<UpdatesScreen> {
  ...
  getHealthNews() async {
    var json;
    try {
      json = await _client.getHealthResponse(dropDownValue);
    } on FetchDataException catch (fde) {
      return fde;
    }
    var articles = json['articles'];
  ...
  refresh() async {
    await Future.delayed(Duration(milliseconds: 800), () {
      setState(() {
        _newsFuture = widget.isHealth ? getHealthNews() : getNews();
      });
    });
  }
